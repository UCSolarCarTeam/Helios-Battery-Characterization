...
def example(resource_name, options, channels, measure, aperture_time,
    ...
    with nidigital.Session(resource_name=resource_name, options=options) as session:
        ...
        dir = os.path.join(os.path.dirname(r'C:\Users\juanc\OneDrive\Documents\Solar_Car_bat\Helios-Battery-Characterization')) #joins one or more path components
        ...
        if source == 'source-current':
            session.channels[channels].ppmu_output_function = nidigital.PPMUOutputFunction.CURRENT #decides whether the ppmu outputs a current or voltage 
            ...
            session.channels[channels].ppmu_source() #starts sourcing voltage or current to DUT
        ...
        if measure == 'current':
            current_measurements = session.channels[channels].ppmu_measure(
                nidigital.PPMUMeasurementType.CURRENT) #Measures current

            print('{:<6} {:<20} {:<10}'.format('Site', 'Pin Name', 'Current')) #prints current data

            for pin, current in zip(pin_info, current_measurements):
                print('{:<6d} {:<20} {:<10f}'.format(
                    pin.site_number, pin.pin_name, current))
        else:
            voltage_measurements = session.channels[channels].ppmu_measure(
                nidigital.PPMUMeasurementType.VOLTAGE) #measures voltage

            print('{:<6} {:<20} {:<10}'.format('Site', 'Pin Name', 'Voltage'))

            for pin, voltage in zip(pin_info, voltage_measurements):
                print('{:<6d} {:<20} {:<10f}'.format(
                    pin.site_number, pin.pin_name, voltage))

        # Disconnect all channels using programmable onboard switching
        session.channels[channels].selected_function = nidigital.SelectedFunction.DISCONNECT


def _main(argsv): #formats the arguments
    ...
    parser.add_argument('-n', '--resource-name', default='PXI1Slot2,PXI1Slot3',
                        help='Resource name of a NI digital pattern instrument, ensure the resource name matches the instrument name in the pinmap file.')
    parser.add_argument('-s', '--simulate', default='True', choices=[
                        'True', 'False'], help='Whether to run on simulated hardware or on real hardware')
    parser.add_argument(
        '-c', '--channels', default='DUTPin1, SystemPin1', help='Channel(s)/Pin(s) to use')
    parser.add_argument('-m', '--measure', default='voltage',
                        choices=['voltage', 'current'], help='Measure voltage or measure current')
    parser.add_argument('-at', '--aperture-time', default=0.000004,
                        type=float, help='Aperture time in seconds')
    subparser = parser.add_subparsers(
        dest='source', help='Sub-command help, by default it measures voltage and does not source')

    source_current = subparser.add_parser(
        'source-current', help='Source current')
    source_current.add_argument('-clr', '--current-level-range',
                                default=0.000002, type=float, help='Current level range in amps')
    source_current.add_argument(
        '-cl', '--current-level', default=0.000002, type=float, help='Current level in amps')
    source_current.add_argument('-vlh', '--voltage-limit-high',
                                default=3.3, type=float, help='Voltage limit high in volts')
    source_current.add_argument('-vll', '--voltage-limit-low',
                                default=0, type=float, help='Voltage limit low in volts')
    source_current.add_argument(
        '-st', '--settling-time', default=0.01, type=float, help='Settling time in seconds')

    source_voltage = subparser.add_parser(
        'source-voltage', help='Source voltage')
    source_voltage.add_argument('-clr', '--current-limit-range',
                                default=0.000002, type=float, help='Current limit range in amps')
    source_voltage.add_argument(
        '-vl', '--voltage-level', default=3.3, type=float, help='Voltage level in volts')
    source_voltage.add_argument(
        '-st', '--settling-time', default=0.01, type=float, help='Settling time in seconds')
    ...
    if args.source == 'source-current':
        ...
        if args.measure == 'current':
            raise ValueError(
                'Cannot measure current on a channel that is not sourcing voltage or current')
        example(
            args.resource_name,
            'Simulate=1, DriverSetup=Model:6571' if args.simulate == 'True' else '',
            args.channels,
...
def test_example():
    ...
    example(resource_name=resource_name, 
            options=options, 
            channels=channels, 
            measure='voltage',
    ...
    with pytest.raises(Exception):
        example(resource_name=resource_name, 
                options=options, channels=channels, measure='current',
                aperture_time=aperture_time)
    ...
    example(resource_name=resource_name, 
           ...
           voltage_limit_high= voltage_limit_high, 
           voltage_limit_low= voltage_limit_low)
